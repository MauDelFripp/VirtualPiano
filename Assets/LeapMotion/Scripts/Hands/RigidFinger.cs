      ...
      for (int i = 0; i < bones.Length; ++i) {
        if (bones[i] != null) {
          ...
          if (capsule != null) {
            ...
            // Update
            capsule.radius = GetBoneWidth(i) / 3f;
            capsule.height = GetBoneLength(i) + GetBoneWidth(i) / 4;
            capsule.center = Vector3.zero;
          }
  
          Rigidbody boneBody = bones[i].GetComponent<Rigidbody>();
          if (boneBody) {
            if(this.bonesMeshes[i] != null)
            {
              Vector3 offsetFromMeshCenter = new Vector3(0, 0, capsule.height/2);
              boneBody.MovePosition(this.bonesMeshes[i].position + this.bones[i].TransformDirection(offsetFromMeshCenter));
            }
            else
            {
